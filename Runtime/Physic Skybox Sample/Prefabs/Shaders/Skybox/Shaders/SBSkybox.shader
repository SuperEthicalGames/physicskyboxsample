Shader "KelvinvanHoorn/Skybox"
{
    Properties
    {
        ...
        _StarBlinkSpeed ("Star blink speed", Range(0,10)) = 1
        _StarBlinkStrength ("Star blink strength", Range(0,1)) = 0.5
        ...
        _ConstellationBlinkSpeed ("Constellation blink speed", Range(0,3)) = 0.35
        _ConstellationBlinkStrength ("Constellation blink strength", Range(0,1)) = 0.1
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURECUBE(_ConstellationCubeMap);
            SAMPLER(sampler_ConstellationCubeMap);
            ...
            float _StarBlinkStrength;
            float3 _ConstellationColor;
            float _ConstellationBlinkSpeed;
            float _ConstellationBlinkStrength;
            ...
            float GetStarBlink(float3 starUVW)
            {
                float seedA = StarHash(starUVW);
                float3 secondarySeedPosition = starUVW.yzx + 17.31;
                float seedB = StarHash(secondarySeedPosition);
                float phase = seedA * 6.28318530718;
                float frequency = lerp(0.35, 1.5, seedB);
                float timeA = _Time.y * _StarBlinkSpeed * frequency + phase;
                float timeB = _Time.y * _StarBlinkSpeed * frequency * 1.73 + phase * 2.31;
                ...
                return lerp(1.0, blinkSignal, _StarBlinkStrength);
            }

            float GetConstellationBlink()
            {
                float wave = sin(_Time.y * _ConstellationBlinkSpeed);
                wave = wave * 0.5 + 0.5;
                return lerp(1.0, wave, _ConstellationBlinkStrength);
            }
            ...
            float4 Fragment(v2f IN) : SV_TARGET
            {
                ...
                float starBlink = GetStarBlink(starUVW);
                ...
                float constellationBlink = GetConstellationBlink();

                constColor *= constellationBlink;
